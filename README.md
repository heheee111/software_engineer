...
- 若 ADRG 无精细映射，则按统一规则追加后缀：`1`（伴 MCC）、`9`（伴 CC）、`5`（无伴）
...
输入：
  主要诊断：C16.301（胃窦恶性肿瘤）
  次要诊断：K66.002（肠粘连）；Z98.800x108（胃术后）；I63.801（腔隙性脑梗死）；K76.807（肝囊肿）
  主要手术：43.7x03（腹腔镜胃大部切除伴胃空肠吻合术）

分组链路：
  C16.301  → 匹配 mdc-digest-tumor → MDCG 消化系统疾病及功能障碍
  43.7x03  → 匹配 adrg-gb2          → GB2 胃、十二指肠大手术
  I63.801  → 命中 CC 列表           → 合并 CC
  GB2 + CC → 查精细映射表           → GB29
```
...
LLM 仅用于「文案润色」，**不参与 DRG 分组决策**。分组结论始终由确定性规则引擎产出，AI 不可修改。

```
解析 EMR → 规则引擎分组 → 得到确定的 DRG 结果 → (可选) LLM 润色文案
                                  ↑
                             AI 在此之后才介入
                             AI 不能改编码结果
```
...
1. **代码架构（最硬约束）** — `agent.py:116`
   ```python
   result = self._engine.group(inp)   # 规则引擎先产出确定结论
   narrative = self._llm_enhance(...) # LLM 只在之后做文字润色
   ```
   LLM 收到的输入是已完成的完整结构化结论（含 MDC/ADRG/DRG 编码及推理链），任务是将其改写为自然语言说明。**架构上决定了 LLM 无权参与分组逻辑。**

2. **System Prompt（辅助约束）** — `agent.py:151`
   - 角色设定：「你是医保 DRG 入组专家」
   - 核心指令：「勿 contradict 给定的编码结论」
   - temperature 压到 0.2，降低模型发散风险

3. **代码兜底（容错）** — `agent.py:174-175`
   - LLM 调用失败（无 API Key / 网络超时 / 返回空）→ 自动回退到结构化原文
   - 系统功能完整可用，LLM 是增强层而非依赖项
...
```bash
git clone https://github.com/heheee111/software_engineer.git
...
### 4. 配置 API Key（可选，不配也能用）

在项目根目录创建 `.env` 文件（或直接编辑已有的 `.env`），填入任一 LLM 服务商的 API Key：
...
不配 API Key 系统也能正常运行，只是 LLM 增强功能会自动跳过。
...
浏览器打开 `http://127.0.0.1:8848`，可见三个功能页签：
...
```bash
# 从 EMR 文件读取并分组
python -m drg_agent.cli examples/slide6_emr.txt

# 从标准输入
echo "主要诊断：J15.0（肺炎）" | python -m drg_agent.cli
```

**方式 C：直接运行 app.py（开发调试）**

```bash
python drg_web/app.py        # 已在 app.py 中处理了路径，可直接运行
python virtual_docs/server.py  # 独立虚拟文档服务（端口 8766）
```

### 6. 运行测试

```bash
# 运行桌面测试用例（需要桌面有 drg_example.json）
